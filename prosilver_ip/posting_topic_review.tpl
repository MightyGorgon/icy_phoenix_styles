...
{IMG_THL}{IMG_THC}<span class="forumlink">{L_TOPIC_REVIEW}</span>{IMG_THR}<table class="forumlinenb">
<tr>
	<td class="row1" style="padding: 0px;">
		<iframe width="100%" height="300" src="{U_REVIEW_TOPIC}" frameborder="0">
<!-- END switch_inline_mode -->
		<table class="forumline">
		<tr><td class="row-header" colspan="2"><span>{L_Post_review}</span></td></tr>
		<tr>
			<th>{L_AUTHOR}</th>
			<th>{L_MESSAGE}</th>
		</tr>
		...
		<tr>
			...
			<td class="row-post tw100pct">
				<table class="s2px p2px">
					<tr>
						<td><div class="post-subject">{postrow.POST_SUBJECT}</div></td>
						<td class="tdalignr">
